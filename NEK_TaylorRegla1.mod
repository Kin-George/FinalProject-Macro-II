...
//Brecha del producto respecto al EE
...
fig_histo1= figure;
histogram(L_valuest);
title('L bajo choques tecnología(Taylor rule sin expectativas)');
xlabel('L');
ylabel('Frequencia');
exportgraphics(fig_histo1, 'histo1.png', 'Resolution', 300);
@# endif
...
xlabel('L');
ylabel('Frequencia');
exportgraphics(fig_histo2, 'histo2.png', 'Resolution', 300);
@# endif 
...
//Para guardar resultados obtenidos
results_tech = zeros(4, length(phis_pi));
results_demand = zeros(4, length(phis_pi));

// Loop incluyendo cada combinación de parámetros
...
////// Resultados shock de tecnología
